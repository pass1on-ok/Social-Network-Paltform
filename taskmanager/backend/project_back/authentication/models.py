...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def _generate_jwt_token(self):
        """
        Generates a JSON Web Token storing this user's identifier,
        with an expiration of 1 day from creation.
        """
        ...
        # Create JWT token with user ID and expiration timestamp
        token = jwt.encode({
            'id': self.pk,
            'exp': exp_timestamp  # Using the timestamp() method
        }, settings.SECRET_KEY, algorithm='HS256')

        # If using older versions of pyjwt, you may need to decode to get a string
        # But in newer versions, this is not required
        # return token.decode('utf-8')  # Uncomment if needed

        return token  # Return the JWT token as is
